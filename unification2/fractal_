...
filename = StripExt(filename) + ".mid"
score.saveAsMidi(filename)
...
Function generatePerlinNoise:Byte[] (l:Int, octaves:Int, p:Float, frq:Float, frqdstr:Float, list:TList, listw:TList)
	Local arr:Byte[l]
	Local tmp:Float
	
	Local totalw:Int = 0
	For Local t:TByte = EachIn listw
		totalw:+t.v
	Next
	
	For Local n:Int = 0 Until l
		tmp = PerlinNoise(n / frq, octaves, p, frqdstr) * totalw
		'arr[n] = TByte(list.ValueAtIndex(Max(Min(tmp * list.Count(), list.Count() - 1), 0))).v
		Local tl:TLink = list.FirstLink(), wcnt:Int
		For Local t:TByte = EachIn listw
			wcnt:+t.v
		...
		arr[n] = TByte(tl.Value()).v
	Next
	Return arr
End Function

Function generateLSystem:Byte[] (itr:Int, start:String, rules:String[], index:String[])
	Local str:String = L_System(itr, start, rules)
	Local arr:Byte[str.length]
	For Local i:Int = 0 Until str.Length
		arr[i] = $ff
		For Local s:String = EachIn index
			If s[0] = str[i]
				s = Mid(s, 3)
				If s[0] > 47 And s[0] < 58
					arr[i] = s.ToInt()
				Else
					arr[i] = NoteFromString(s)
				End If
				Exit
			End If
		Next
...
Type TmidiChannel
	...
	Method fromArrays(parr:Byte[], varr:Byte[])
		arrlength = Min(parr.length, varr.length)
		Local arr:TmidiNote[arrlength]
		notes = arr
		
		For Local i:Int = 0 Until arrlength
			notes[i] = New TmidiNote
			notes[i].note = parr[i]
			notes[i].length = varr[i]
		Next
	End Method
End Type
			...
			While ge < Len(file)
				...
				Select title
					Case "general"
						...
						cs = Instr(content, "seed")
						If cs <> 0
							cs:+5
							ce = Instr(content, ";", cs)
							seed = Mid(content, cs, ce - cs).Toint()
						End If
					...
					Case "channel"
						...
						Local ptype:Int, vtype:Int
						Local parr:Byte[], varr:Byte[]
						...
						cs = Instr(content, "pitch_type")
						If cs <> 0
							cs:+11
							ce = Instr(content, ";", cs)
							ptype = Mid(content, cs, ce - cs).ToInt()
						End If
						
						If ptype = 0
							Local pitr:Int, pstart:String, prules:String[], pindex:String[]
							...
							parr = generateLSystem(pitr, pstart, prules, pindex)
						Else If ptype = 1
							Local plength:Int, poctaves:Int, pp:Float, pfrq:Float, pfrqdstr:Float, plist:TList, plistw:TList
							
							cs = Instr(content, "pitch_seed")
							If cs <> 0
								cs:+11
								ce = Instr(content, ";", cs)
								SeedRnd(Mid(content, cs, ce - cs).ToInt() + seed)
							Else
								SeedRnd(seed)
							End If
							
							cs = Instr(content, "pitch_length")
							If cs <> 0
								cs:+13
								ce = Instr(content, ";", cs)
								plength = Mid(content, cs, ce - cs).ToInt()
							End If
							
							cs = Instr(content, "pitch_octaves")
							If cs <> 0
								cs:+14
								ce = Instr(content, ";", cs)
								poctaves = Mid(content, cs, ce - cs).ToInt()
							End If
							
							cs = Instr(content, "pitch_p")
							If cs <> 0
								cs:+8
								ce = Instr(content, ";", cs)
								pp = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "pitch_frq")
							If cs <> 0
								cs:+10
								ce = Instr(content, ";", cs)
								pfrq = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "pitch_frqdstr")
							If cs <> 0
								cs:+14
								ce = Instr(content, ";", cs)
								pfrqdstr = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "pitch_list")
							If cs <> 0
								cs:+11
								ce = Instr(content, ";", cs)
								...
								For Local str:String = EachIn tmparr
									...
									If cs <> 0
										cs:+1
										ce = Instr(str, "]")
										plistw.AddLast(TByte.N(Mid(str, cs, ce - cs).ToInt()))
									Else
										plistw.AddLast(TByte.N(1))
									End If
									
									If str[0] > 47 And str[0] < 58
										plist.AddLast(TByte.N(str.ToInt()))
									Else
										plist.AddLast(TByte.N(NoteFromString(str)))
									End If
								Next
							End If
							
							parr = generatePerlinNoise(plength, poctaves, pp, pfrq, pfrqdstr, plist, plistw)
						End If
						
						
						cs = Instr(content, "value_type")
						If cs <> 0
							cs:+11
							ce = Instr(content, ";", cs)
							vtype = Mid(content, cs, ce - cs).ToInt()
						End If
						
						If vtype = 0
							Local vitr:Int, vstart:String, vrules:String[], vindex:String[]
							...
							varr = generateLSystem(vitr, vstart, vrules, vindex)
						Else If vtype = 1
							Local vlength:Int, voctaves:Int, vp:Float, vfrq:Float, vfrqdstr:Float, vlist:TList, vlistw:TList
							
							cs = Instr(content, "value_seed")
							If cs <> 0
								cs:+11
								ce = Instr(content, ";", cs)
								SeedRnd(Mid(content, cs, ce - cs).ToInt() + seed)
							...
							cs = Instr(content, "value_length")
							If cs <> 0
								cs:+13
								ce = Instr(content, ";", cs)
								vlength = Mid(content, cs, ce - cs).ToInt()
							End If
							
							cs = Instr(content, "value_octaves")
							If cs <> 0
								cs:+14
								ce = Instr(content, ";", cs)
								voctaves = Mid(content, cs, ce - cs).ToInt()
							End If
							
							cs = Instr(content, "value_p")
							If cs <> 0
								cs:+8
								ce = Instr(content, ";", cs)
								vp = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "value_frq")
							If cs <> 0
								cs:+10
								ce = Instr(content, ";", cs)
								vfrq = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "value_frqdstr")
							If cs <> 0
								cs:+14
								ce = Instr(content, ";", cs)
								vfrqdstr = Mid(content, cs, ce - cs).ToFloat()
							End If
							
							cs = Instr(content, "value_list")
							If cs <> 0
								cs:+11
								ce = Instr(content, ";", cs)
								Local tmparr:String[] = Mid(content, cs, ce - cs).Split(",")
								...
								For Local str:String = EachIn tmparr
									...
									If cs <> 0
										cs:+1
										ce = Instr(str, "]")
										vlistw.AddLast(TByte.N(Mid(str, cs, ce - cs).ToInt()))
									Else
										vlistw.AddLast(TByte.N(1))
									End If
									vlist.AddLast(TByte.N(str.ToInt()))
								Next
							End If
							
							
							varr = generatePerlinNoise(vlength, voctaves, vp, vfrq, vfrqdstr, vlist, vlistw)
						End If
						
						
						c.fromArrays(parr, varr)
				End Select
			Wend
		Else
